    ...
    <TableContext.Provider value={{ onRowClick: onRowClick as (row: unknown) => void, isRowClickable }}>
      <div
        className={clsx(
          ...
          framed
            ? 'border border-border-soft bg-base-panel px-3 py-3 shadow-elevation-card backdrop-blur-2xl'
            : 'px-3 py-3',
          className
        )}
      >
        <table
          className="min-w-full text-[13px] text-text-primary border-collapse [&>thead>tr>th]:py-3 [&>thead>tr>th]:px-4 [&>thead>tr>th]:text-[11px] [&>thead>tr>th]:font-semibold [&>thead>tr>th]:uppercase [&>thead>tr>th]:tracking-[0.14em] [&>thead>tr>th]:text-text-secondary [&>thead]:bg-white/[0.03] [&>thead]:border-b [&>thead]:border-border-soft [&>tbody>tr]:border-b [&>tbody>tr]:border-border-soft [&>tbody>tr:last-child]:border-b-0 [&>tbody>tr>td]:py-3 [&>tbody>tr>td]:px-4 [&>tbody>tr>td]:align-middle [&>tbody>tr]:transition-colors [&>tbody>tr]:duration-150 [&>tbody>tr:hover]:bg-white/[0.02]"
        >
          {children}
        </table>
      </div>
