  ...
  return (
    <div className={clsx('relative inline-flex items-center gap-1 text-xs text-text-muted', className)}>
      ...
      <button
        ...
        className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-bg-surface text-accent-primary border border-border-subtle shadow-soft transition hover:shadow-lifted"
      >
      ...
      {open && (
        <div className="absolute top-full left-0 mt-2 w-72 p-4 rounded-xl bg-bg-surface border border-border-subtle shadow-lifted z-20 text-sm">
          <p className="text-text-primary leading-snug">{label}</p>
        </div>
      )}
    </div>
  );
